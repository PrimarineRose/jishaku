...
class JishakuBase(commands.Cog):  # pylint: disable=too-many-public-methods
    ...
    async def jsk_hide(self, ctx: commands.Context):
        ...
        if self.jsk.hidden:
            emb = discord.Embed(title="", description = "Jishaku is already hidden.", color=0xaf85ff)
            return await ctx.send(embed=emb)

        self.jsk.hidden = True
        emb = discord.Embed(title="", description = "Jishaku is now hidden.", color=0xaf85ff)
    ...
    async def jsk_show(self, ctx: commands.Context):
        ...
        if not self.jsk.hidden:
            emb = discord.Embed(title="", description = "Jishaku is already visible.", color=0xaf85ff)
            return await ctx.send(embed=emb)

        self.jsk.hidden = False
        emb = discord.Embed(title="", description = "Jishaku is now visible.", color=0xaf85ff)
        await ctx.send(embed=emb)
    ...
    async def jsk_tasks(self, ctx: commands.Context):
        ...
        if not self.tasks:
            emb = discord.Embed(title="", description = "No currently running tasks.", color=0xaf85ff)
            return await ctx.send(embed=emb)
    ...
    async def jsk_cancel(self, ctx: commands.Context, *, index: int):
        ...
        if not self.tasks:
            emb = discord.Embed(title="", description = "No tasks to cancel.", color=0xaf85ff)
            return await ctx.send(embed=emb)
        ...
        if index == -1:
            ...
            if task:
                ...
                emb = discord.Embed(title="", description = "Unknown task.", color=0xaf85ff)
                return await ctx.send(embed=emb)

        task.task.cancel()
        emb = discord.Embed(title="", description = f"Cancelled task {task.index}: `{task.ctx.command.qualified_name}`,"
                              f" invoked at {task.ctx.message.created_at.strftime('%Y-%m-%d %H:%M:%S')} UTC", color=0xaf85ff)
        return await ctx.send(embed=emb)
    ...
    async def jsk_load(self, ctx: commands.Context, *extensions: ExtensionConverter):
        ...
        for extension in itertools.chain(*extensions):
            method, icon = (
                (self.bot.reload_extension, "<:checkmark:810486084487610409>")
        ...
        for page in paginator.pages:
            emb = discord.Embed(title="", description = page, color=0xaf85ff)
            await ctx.send(embed=emb)
    ...
    async def jsk_sudo(self, ctx: commands.Context, *, command_string: str):
        ...
        if alt_ctx.command is None:
            emb = discord.Embed(title="", description = f'Command "{alt_ctx.invoked_with}" is not found', color=0xaf85ff)
            return await ctx.send(embed=emb)
    ...
    async def jsk_debug(self, ctx: commands.Context, *, command_string: str):
        ...
        emb = discord.Embed(title="", description = f"Command `{alt_ctx.command.qualified_name}` finished in {end - start:.3f}s.", color=0xaf85ff)
        return await ctx.send(embed=emb)
